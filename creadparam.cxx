...
static int FadcFlag=0;                       //@< will the electronics be simulated
static float Fadc_bin = 12;                    //@< the total bin number of fadc
static float Fadc_bin_length = 20;             //@< the bin length of each fadc
static int NsbFlag = 1;                        //@< the flag of Nsb simulated or not
static float Nsb = 0.;                         //@< the night sky background
static float TriggerSigma = 4;                 //@< the ratio of signal to noise;
static float FixTriggerThreshold = 5;          //@< if NSB is not simulated, the fix trigger threshold is used
using namespace std;
 ...
 while (! is_end) {
   ...
   switch ( i ) {
     ...
     case fadc_bin_length:
         ...
         break;
     case nsb:
         sscanf(line,"%s %s %f %f",token,token2,&x, &y);
         if(strcmp(token2,"T")==0||strcmp(token2,"TRUE")==0){
           NsbFlag = TRUE;
           Nsb = x; 
           TriggerSigma = y;
printf("Triggersigma = %f\n",y);
         }
         else{
           NsbFlag = FALSE;
           FixTriggerThreshold = x;
         }
         break;
     case end_file:
...
int  WReadConfig::GetNSBFlag()
{
  return NsbFlag;
}
float WReadConfig::GetFixTriggerThreshold()
{
   return FixTriggerThreshold;
}
float WReadConfig::GetTriggerSigma()
{
   return TriggerSigma;
}
int WReadConfig::GetFadcFlag()
{
   return FadcFlag;
